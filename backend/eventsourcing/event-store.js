...
class EventStore {
  constructor({
    ...
    snapshotThreshold = SNAPSHOT_THRESHOLD_ENV,
    client = null,
  } = {}) {
    ...
    this._client = client || supabaseAdmin || supabase; // defaults to service-role admin client
    this._core = null;
  ...
  async handleCommand(command) {
    ...
    try {
      ...
      const result = await context.with(trace.setSpan(context.active(), span), async () => {
        ...
        // Store events under optimistic concurrency.
        const storedEvents = [];
  ...
  async getOrderList(filters = {}) {
    let query = this._client.from('orders_read_model').select('*');
